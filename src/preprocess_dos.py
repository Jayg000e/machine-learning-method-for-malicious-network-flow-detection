...
def prepare_data(data_dir,dst_dir):
    ...
    def label(flowtype):
        if flowtype=='Benign':
            return 0
        elif flowtype=='DoS attacks-GoldenEye':
            return 1
        else:
            return 2
    ...
    normal_index,goldeneye_index,slowloris_index=np.arange(num_capture)[capture.Label==0], \
    ...
    normal_index,ftp_index,ssh_index=np.random.choice(normal_index,samples,replace=False), \
                                     np.random.choice(goldeneye_index,samples,replace=False), \
                                     np.random.choice(slowloris_index,samples,replace=False)

    np.random.shuffle(normal_index)
    np.random.shuffle(goldeneye_index)
    np.random.shuffle(slowloris_index)
    ...
    capture_test=pd.concat([capture.loc[normal_index[int(train_sample_rate*samples):]],
                            capture.loc[goldeneye_index[int(train_sample_rate*samples):]],
                            capture.loc[slowloris_index[int(train_sample_rate*samples):]]
                            ],ignore_index=True)
