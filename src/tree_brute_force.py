...
from preprocess_brute_force import prepare_data
import os
import pickle
import numpy as np
...
seed=100
data_dir='/home/share/guojia/botnet_detection/02-14-2018.csv'
dst_dir='brute_force_attack_preprocess.pickle'
result_dir=os.path.join('result','tree_brute_force')
...
if attack_classification:
    ...
    class_names=['benign','ftp_brute_force','ssh_brute_force']
else:
    label_train,label_test=label_train['attack'],label_test['attack']
    class_names=['normal','brute_force_attack']


clf = tree.DecisionTreeClassifier(max_depth=2,class_weight='balanced', random_state=seed)
...
if not attack_classification:
    ...
    plt.savefig(os.path.join(result_dir,'ROC_tree_brute_force.png'))
else:
    metrics.ConfusionMatrixDisplay.from_predictions(label_test, Prediction,display_labels=class_names)
    plt.savefig(os.path.join(result_dir,'confusion_matrix_tree_brute_force.png'))
...
dst=os.path.join(result_dir,'decision_tree_'+'brute_force_classification') if attack_classification else os.path.join(result_dir,'decision_tree_'+'brute_force_detection')
graph.render(dst)
