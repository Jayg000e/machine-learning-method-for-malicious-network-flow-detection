...
def prepare_data(data_dir,dst_dir):
    ...
    def label(flowtype):
        if flowtype=='Benign':
            return 0
        elif flowtype=='DDOS attack-HOIC':
            return 1
        else:
            return 2
    ...
    normal_index,DDoS_HOIC_index,DDOS_LOIC_UDP_index=np.arange(num_capture)[capture.Label==0], \
    ...
    normal_index,ftp_index,ssh_index=np.random.choice(normal_index,samples,replace=False), \
                                     np.random.choice(DDoS_HOIC_index,samples,replace=False), \
                                     np.random.choice(DDOS_LOIC_UDP_index,samples,replace=False)

    np.random.shuffle(normal_index)
    np.random.shuffle(DDoS_HOIC_index)
    np.random.shuffle(DDOS_LOIC_UDP_index)
    ...
    capture_test=pd.concat([capture.loc[normal_index[int(train_sample_rate*samples):]],
                            capture.loc[DDoS_HOIC_index[int(train_sample_rate*samples):]],
                            capture.loc[DDOS_LOIC_UDP_index[int(train_sample_rate*samples):]]
                            ],ignore_index=True)
